...
const pool = new Pool({
  ...
  // Neon free tier suspends after inactivity and can take >60s to wake.
  // Allow up to 90s for the control plane to resume the database.
  connectionTimeoutMillis: 90000,
});

// Retry connect up to 3 times with a 5s pause — handles Neon cold-start
// "Control plane request failed" transient errors.
async function connectWithRetry(retries = 3, delayMs = 5000) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      return await pool.connect();
    } catch (err) {
      const isControlPlane = err.message && err.message.includes('Control plane request failed');
      if (attempt < retries && isControlPlane) {
        console.log(`DB connect attempt ${attempt} failed (control plane), retrying in ${delayMs}ms...`);
